// ════════════════════════════════════════════════════════════════════════════
//  BROWNSTONE — Daily Search Term Report  v25
//  Changes vs v24:
//  - FIXES "↩ Remove" button not sticking. Two bugs: (1) ex map was built
//    before the removal so the email still showed the term as blocked;
//    (2) classifyTerm would immediately re-block it if it matched an auto-
//    block rule. Fix: delete ex[t] + add to keepSet after unblocking, so the
//    term is treated as permanently approved and never auto-re-blocked.
//    Requires matching Apps Script update (unblock handler now adds to keeps
//    instead of deleting from keeps).
// ════════════════════════════════════════════════════════════════════════════
    ...
    if (resp.getResponseCode() === 200) {
      ...
      if (commands && commands.length > 0) {
        ...
        for (var i = 0; i < commands.length; i++) {
          ...
          if (c.a === 'unblock') { removeNegAllCampaigns(t, campaignIds); delete ex[t]; keepSet[t] = true; }
        }
...
function btn(label, term, action, bg) {
  // Pack action|term|secret into ONE base64 token so the URL has no "&".
  // Gmail's link wrapper splits tapped links on the first "&" and would
  // otherwise drop term+secret, producing "can't open the file".
  var payload = action + '~~' + term + '~~' + CONFIG.WEBAPP_SECRET;
  var token   = Utilities.base64EncodeWebSafe(Utilities.newBlob(payload).getBytes());
  var url     = CONFIG.WEBAPP_URL + '?d=' + encodeURIComponent(token);
...
function main() {
  ...
  var allMap = {}, yMap = {}, yExcluded = {}, tMap = {}, tExcluded = {};

  // Date range now includes today so live/partial data is visible
  var q = 'SELECT search_term_view.search_term, metrics.cost_micros, metrics.clicks, '
    ...
    + 'WHERE segments.date BETWEEN "' + fmtDash(start30) + '" AND "' + todayStr + '"';
  ...
  while (res.hasNext()) {
    ...
    if (status === 'EXCLUDED') {
      ...
      if (rDate === yStr) {
        ...
        if (!tExcluded[term]) tExcluded[term] = { term:term, cost:0, clicks:0, conv:0 };
        tExcluded[term].cost += cost; tExcluded[term].clicks += clicks; tExcluded[term].conv += conv;
      }
      continue;
    }
    ...
    if (rDate === todayStr) {
      if (!tMap[term]) tMap[term] = { term:term, cost:0, clicks:0, conv:0 };
      tMap[term].cost += cost; tMap[term].clicks += clicks; tMap[term].conv += conv;
    }
  }

  // Classify and apply tags to allMap, yMap, and tMap
  for (var t in allMap) {
    ...
    if (tMap[t]) { tMap[t].tag = cls.tag; tMap[t].reason = cls.reason; }
  }

  var thirtyDayList = sortedArray(allMap);

  // Today list: new terms + already-blocked
  var todayNewArr     = sortedArray(tMap);
  var todayBlockedArr = sortedArray(tExcluded);
  for (var bi = 0; bi < todayBlockedArr.length; bi++) {
    todayBlockedArr[bi].tag    = 'already_blocked';
    todayBlockedArr[bi].reason = 'Already in your negative keyword list';
  }
  var todayList = todayNewArr.concat(todayBlockedArr);

  // Yesterday list: new terms + already-blocked
  ...
  Logger.log('Campaigns: '+campaignIds.length+' | Rows: '+counters.checked
    +' | Auto-blocked: '+counters.blocked+' | Flagged: '+counters.review
    +' | Today new: '+todayNewArr.length+' | Today blocked: '+todayBlockedArr.length
    +' | Yesterday new: '+newTermsArr.length+' | Yesterday blocked: '+alreadyBlockedArr.length);

  sendReport(counters, today, yesterday,
             todayList, todayNewArr.length, todayBlockedArr.length,
             yesterdayList, newTermsArr.length, alreadyBlockedArr.length,
...
function sendReport(counters, today, yesterday,
                    todayList, todayNewCount, todayBlockedCount,
                    yesterdayList, newCount, blockedCount,
  ...
  // Today section
  var todayTotal = todayNewCount + todayBlockedCount;
  var todayLabel = todayTotal > 0
    ? (todayTotal + ' total (' + todayNewCount + ' new'
       + (todayBlockedCount > 0 ? ', ' + todayBlockedCount + ' already blocked' : '')
       + ')')
    : '0 terms so far';
  var tHtml = todayList.length === 0
    ? '<p style="color:#888;font-style:italic">No terms yet today — '
      + 'search term data typically appears 3–6 hours after it fires.</p>'
    : buildTable(todayList, '13px');

  // Yesterday section
  var yTotal = newCount + blockedCount;
  var yLabel = yTotal + ' total (' + newCount + ' new'
  ...
  var body = '<div style="font-family:sans-serif;max-width:700px;color:#111">'
    ...
    + '<h3 style="margin-bottom:2px">🔴 Today — ' + formatDate(today)
    + ' &nbsp;<span style="font-weight:normal;font-size:13px;color:#555">(' + todayLabel
    + ' — live, partial)</span></h3>'
    + '<p style="color:#888;font-size:11px;margin:0 0 8px">Live data — reruns each time you run the script. '
    + 'May not show the full day yet (data has a 3–6 hr delay).</p>'
    + tHtml
    // YESTERDAY
    ...
    + 'already in your negative list. '
    + 'Compare this total to Google Ads → Search Terms filtered to '
    + formatDate(yesterday) + ' to verify coverage.</p>'
    + yHtml
    // 30 DAYS
    + '<hr style="border:none;border-top:2px solid #e5e7eb;margin:24px 0 12px">'
    ...
    + negHtml
    + '<p style="color:#bbb;font-size:11px;margin-top:20px">v25 — Brownstone HVAC</p></div>';
