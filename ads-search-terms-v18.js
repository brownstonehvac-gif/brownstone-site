// ════════════════════════════════════════════════════════════════════════════
//  BROWNSTONE — Daily Search Term Report  v18  (show blocked yesterday terms + button feedback)
// ════════════════════════════════════════════════════════════════════════════

var CONFIG = {
  EMAIL:             'hvac4lif@gmail.com',
  CAMPAIGN_IDS:      [23097095929, 23064032078],
  MIN_COST_TO_FLAG:  5.00,
  ...
  ALWAYS_BLOCK_KEYWORDS: [
    'costco','home depot','lowes',"lowe's",'amazon',
    'how to','diy','do it yourself','youtube','video tutorial',
    'tech support','technical support','service manual','warranty claim',
    'parts lookup','distributor','supplier','wholesale','for sale',
    'serial number','model number','part number',
    'long island city','queens','manhattan','bronx','staten island',
    'hiring','jobs','career','employment','salary',
    'apprentice','training','school','certification','certificate',
    'yelp','angi','thumbtack','homeadvisor',
    'permit','rebate','energy star','what is hvac','hvac meaning',
    '311','free air conditioner','free ac',
    'portable ac','portable air conditioner','portable unit',
    'portable cooler','portable cooling',
    'window ac','window unit','window air conditioner',
    'window mounted ac','window mount ac',
    'room ac','room air conditioner',
    'floor ac','freestanding ac',
    'through the wall ac','thru wall ac'
  ],
  ...
  GENERIC_LEAD: [
    ...
    'company','companies','contractor','my'
  ],
  BAD_KEYWORDS: [
    'goodman','rheem','carrier northeast','abco','daikin air conditioner',
    'buy','btu','specs','manual','refrigerant','freon','r22','r410a',
    'ton unit','ac size','calculator'
  ],
  REPAIR_INTENT_WORDS: [
...
function processWebAppCommands(ex) {
  var keepSet = {};
  var processed = 0;

  try {
    // Step 1 — load permanent keep list from web app (persists forever)
    ...
    // Step 2 — process pending queue (block / unblock actions from email buttons)
    ...
    if (resp.getResponseCode() === 200) {
      ...
      if (commands && commands.length > 0) {
        ...
        for (var i = 0; i < commands.length; i++) {
          ...
          if (!t) continue;
          if (c.a === 'block')   { delete keepSet[t]; blockTermAllCampaigns(t, ex); }
          if (c.a === 'unblock') { removeNegAllCampaigns(t); }
        }
        UrlFetchApp.fetch(
...
function getExistingNegatives() {
  var ex = {};
  for (var c = 0; c < CONFIG.CAMPAIGN_IDS.length; c++) {
    var it = AdsApp.campaigns().withIds([CONFIG.CAMPAIGN_IDS[c]]).get();
...
function blockTermAllCampaigns(term, ex) {
  var c = term.toLowerCase().trim();
  if (ex[c]) return false;
  var did = false;
  for (var i = 0; i < CONFIG.CAMPAIGN_IDS.length; i++) {
    var ci = AdsApp.campaigns().withIds([CONFIG.CAMPAIGN_IDS[i]]).get();
    if (ci.hasNext()) { ci.next().createNegativeKeyword('[' + c + ']'); ex[c] = true; did = true; }
  }
  return did;
}

function removeNegAllCampaigns(term) {
...
function classifyTerm(term, totalCost, totalConv, ex, counters, keepSet) {
  ...
  if (getMatch(term, CONFIG.MANUAL_BLOCK)) {
    blockTermAllCampaigns(term, ex); counters.blocked++;
  ...
  if (inSafelist(term)) {
    ...
    if (am) {
      ...
      if (blockTermAllCampaigns(term, ex)) counters.blocked++;
    } else {
...
function main() {
  ...
  var ex         = getExistingNegatives();
  var webResult  = processWebAppCommands(ex);
  ...
  var q = 'SELECT search_term_view.search_term, metrics.cost_micros, metrics.clicks, '
    ...
    + 'AND campaign.id IN (' + CONFIG.CAMPAIGN_IDS.join(',') + ')';
  ...
  while (res.hasNext()) {
    ...
    if (status === 'EXCLUDED') {
      ...
      // FIX: track blocked terms that still fired yesterday so we can show them in the email
      if (rDate === yStr) {
  ...
  for (var t in allMap) {
    ...
    var entry = allMap[t];
    var cls   = classifyTerm(t, entry.cost, entry.conv, ex, counters, keepSet);
  ...
  Logger.log('Checked '+counters.checked+' | Blocked '+counters.blocked+' | Review '+counters.review+' | Already blocked (hidden) '+counters.alreadyBlocked);
  sendReport(counters, today, yesterday, yesterdayList, thirtyDayList, ex, yExcludedList);
}
...
function sendReport(counters, today, yesterday, yesterdayList, thirtyDayList, ex, yExcludedList) {
  ...
  // Collapsible table of already-blocked terms that still fired yesterday
  var yExcHtml = '';
  ...
  // Show feedback if button actions were applied this run
  ...
  var body = '<div style="font-family:sans-serif;max-width:700px;color:#111">'
    ...
    + negHtml
    + '<p style="color:#bbb;font-size:11px;margin-top:20px">v18 — Brownstone HVAC</p></div>';
